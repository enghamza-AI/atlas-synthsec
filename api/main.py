...
app = FastAPI(
    title="Atlas — Marketing Intelligence API",
    description=(
        "Local-only service wrapping the Atlas pipeline for reuse "
...
def run_analysis(request: RunAnalysisRequest) -> dict:
    ...
    return {
        "n_campaigns": output["n_campaigns"],
        "model_mae": output["model_mae"],
        "silhouette_avg": output["silhouette_avg"],
        "pct_measured": output["pct_measured"],
        "campaigns": result_df.to_dict(orient="records"),
    }
